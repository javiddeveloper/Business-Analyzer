...
async function runReview({ mr, changes, projectPath, mrIid, headSha, job, signal }) {
  ...
  if (agentWanted && projectPath && headSha) {
    try {
      ...
      const agentResult = await agentReview.review({ mr, changes, worktreePath, model: secret('CLAUDE_MODEL') || '', signal });
      ...
      for (const candidate of listReadyEngines(provider)) {
        ...
        const result = await runBatchReview({ mr, changes, projectPath, mrIid, headSha, signal, provider: candidate });
...
async function runBatchReview({ mr, changes, projectPath, mrIid, headSha, signal, provider }) {
  ...
  const result = await reviewer.review({ mr, changes, fileContents: local.fileContents, signal, provider });
  if (local.warning) {
    result.findings = reviewer.sortFindings([
      ...result.findings,
