...
// the dashboard's "امتیاز مدیر" month picker is built from.
...
// The manager's rating shaped for a reader rather than for the form: which
// month it belongs to, the 0-100 headline, every parameter as it was entered,
// and the note. Null when nobody has rated this person yet — whether that is
// a blank row or no row at all is the caller's call, not ours.
function summary(author) {
  const months = monthsFor(author);
  const keys = Object.keys(months).sort().reverse();
  if (!keys.length) return null;
  const month = keys[0];
  const rating = months[month];
  return {
    month,
    overall: overall(rating),
    note: rating.note || '',
    updatedAt: rating.updatedAt || null,
    params: PARAMS.map((p) => ({ key: p.key, label: p.label, value: rating.scores[p.key] ?? null })),
  };
}

module.exports = { PARAMS, get, set, overall, summary, listMonths, latest, currentMonthKey };
