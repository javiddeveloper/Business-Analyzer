...
// projectId, when given, is a configured project's own GitLab (lib/projects.js
// connections.gitlab) — a different self-hosted instance than secrets.env's,
// not just a different project id on the same one. Omitted, this is exactly
// the single-GitLab behaviour every caller had before per-project overrides
// existed.
function gitlabBase(projectId) {
  return (secret('GITLAB_URL', projectId) || 'https://gitlab.com').replace(/\/$/, '');
}
...
// `projectId` on options (not a positional arg) so every call site below
// keeps reading as "path, options" — it's set to whichever workspace project
// this request is already scoped to, or left off to use secrets.env's single
// GitLab entirely as before.
async function apiFetch(pathname, { projectId, ...options } = {}) {
  const url = `${gitlabBase(projectId)}/api/v4${pathname}`;
  const headers = Object.assign(
    { 'PRIVATE-TOKEN': secret('GITLAB_TOKEN', projectId), 'Content-Type': 'application/json' },
    options.headers || {}
  );
...
function getMergeRequestChanges(projectId, mrIid) {
  return apiFetch(`/projects/${encodeURIComponent(projectId)}/merge_requests/${mrIid}/changes`, { projectId });
}
...
function listDiscussions(projectId, mrIid) {
  return apiFetch(`/projects/${encodeURIComponent(projectId)}/merge_requests/${mrIid}/discussions?per_page=100`, { projectId });
}
...
function getCurrentUser(options, projectId) {
  return apiFetch('/user', { ...options, projectId });
}
...
function listOpenMergeRequests(projectId) {
  ...
  return id
    ? apiFetch(`/projects/${encodeURIComponent(id)}/merge_requests?${q}`, { projectId: id })
...
async function apiFetchAll(pathname, { maxPages = 5, projectId } = {}) {
  const sep = pathname.includes('?') ? '&' : '?';
  const perPage = 100;
  let all = [];
  for (let page = 1; page <= maxPages; page++) {
    const batch = await apiFetch(`${pathname}${sep}per_page=${perPage}&page=${page}`, { projectId });
...
function listProjectMembers(projectId) {
  ...
  return apiFetchAll(`/projects/${encodeURIComponent(id)}/members/all`, { projectId: id });
}
...
//
// includeMaintainers widens that to Maintainers and Owners as well, for
// teams whose lead ships features rather than only reviewing them. Passed in
// by the caller rather than read from settings here: this module talks to
// GitLab and knows nothing about the dashboard's switches, and giving it a
// second source of truth for its own behaviour is how that stays true.
async function listAllAuthors(projectId, { includeMaintainers = false } = {}) {
  const id = projectId || secret('GITLAB_PROJECT_ID');
  const q = 'state=all&order_by=created_at&sort=desc';
  const mrs = await apiFetchAll(
    id ? `/projects/${encodeURIComponent(id)}/merge_requests?${q}` : `/merge_requests?scope=all&${q}`,
    { maxPages: 5, projectId: id }
  );
  const seen = new Map();
  for (const mr of mrs) {
  ...
  // >= rather than a second equality test: Owner (50) is above Maintainer
  // (40), and an Owner who writes code belongs in the roster for exactly the
  // same reason a Maintainer does.
  const allowed = new Set(
    members
      .filter((m) => (includeMaintainers ? m.access_level >= DEVELOPER_ACCESS_LEVEL : m.access_level === DEVELOPER_ACCESS_LEVEL))
      .map((m) => m.username)
  );
  return authors.filter((a) => allowed.has(a.username));
}
