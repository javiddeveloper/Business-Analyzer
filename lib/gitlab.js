...
// when GITLAB_PROJECT_ID is set (roles are a project-scoped concept; there's
// no single membership list across "every project this token can see").
function listProjectMembers() {
  const projectId = secret('GITLAB_PROJECT_ID');
  if (!projectId) return Promise.resolve(null);
  return apiFetchAll(`/projects/${encodeURIComponent(projectId)}/members/all`);
}
...
// open right now, unlike listOpenMergeRequests's audience) — filtered down
// to actual Developer-role members when GITLAB_PROJECT_ID is set. Without a
