...
function taskReasons(task, now) {
  ...
  if (devScore.needsTimeLog(task) && !(task.spentHours > 0)) reasons.push('no-worklog');
...
function buildRow({ username, name, analytics, now = Date.now() }) {
  ...
  const missingWorklog = tasks.filter((t) => devScore.needsTimeLog(t) && !(t.spentHours > 0));
  ...
  const row = {
    ...
    // Hours logged in Jira (lib/worklogHours.js) — null, not 0, when Jira
    // isn't configured: "no data" must not read as "worked zero hours".
    hoursThisWeek: analytics && analytics.worklogHours ? analytics.worklogHours.thisWeek.hours : null,
    capacityThisWeekSoFar: analytics && analytics.worklogHours ? analytics.worklogHours.thisWeek.capacitySoFarHours : null,
    hoursThisMonth: analytics && analytics.worklogHours ? analytics.worklogHours.thisMonth.hours : null,
    capacityThisMonthSoFar: analytics && analytics.worklogHours ? analytics.worklogHours.thisMonth.capacitySoFarHours : null,
