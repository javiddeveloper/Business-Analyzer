...
function summary({ days = 30, now = Date.now() } = {}) {
  ...
  // A rolling 7x24h window, not "since Saturday": the dashboard is asking
  // "how much have we burned lately", and a calendar week answers that with
  // a number that collapses every time the week rolls over.
  const weekSince = now - 7 * 86400000;
  ...
  for (const r of inWindow) {
    ...
    if (r.at >= weekSince) {
      weekReviews++;
      weekPromptTokens += r.promptTokens;
      weekCompletionTokens += r.completionTokens;
    }
  }

  // The most recent review's prompt size. A context window is a per-request
  // limit, not something that fills up over a day — so the only honest way to
  // draw it as a bar is "how much of one request's allowance did the last
  // review actually use".
  ...
  return {
    ...
    // True when the caller's window is 7 days or shorter, in which case
    // `week` is just `total` under another name.
    weekIsWholeWindow: days <= 7,
    reviews,
    promptTokens,
    completionTokens,
    ...
    week: {
      ...
      totalTokens: weekPromptTokens + weekCompletionTokens,
    },
