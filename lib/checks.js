...
function checkSize(files, skipped) {
  ...
  if (files.length > 40 || totals.added > 1500) {
    out.push(finding({
      file: null,
      line: null,
      // Low on purpose, for the same reason task-mismatch is pinned Low in
      // reviewer.js: decide() blocks on High/Medium, and "this MR is big" is
      // not a defect in the code — it is a statement about the shape of the
      // change. Held at Medium it made an MR whose only real findings were
      // Low come back REQUEST_CHANGES on file count alone, which reads to
      // the author as "your code is broken" and is not what was meant.
      //
      // Nor is blocking the lever that fits: by the time this fires the
      // large diff already exists, and refusing to approve it does not make
      // it smaller — it just stalls work that may be perfectly correct.
      // Size already carries a real consequence elsewhere: the maintainer's
      // L rating feeds devScore.mrSize (weight 15), which is where the team
      // decided big merge requests should cost something. Blocking here as
      // well was charging twice for one thing, with the wrong instrument.
      severity: 'Low',
      category: 'process',
