...
const deliveryMetrics = require('./deliveryMetrics');
...
// The round trip is a negative mark, so it only counts once a maintainer has
// added the review file for that MR — and stops counting the moment the file
// goes away. See lib/reviewSignoff.js for why "the file exists" was never
// enough on its own: this tool writes that file itself on every run.
//
// Loaded once per analytics build (one `git log`, one members call), not per
// merge request.
async function loadSignoffContext(projectIds) {
  const ctx = { signoffs: new Map(), maintainers: [], byProject: new Map() };
  try {
    const members = await gitlab.listProjectMembers();
    ctx.maintainers = reviewSignoff.maintainersFrom(members);
  } catch (e) {
    ctx.maintainers = [];
  }
  for (const id of projectIds) {
    const projectPath = projects.getProjectPath(id) || secret('PROJECT_PATH');
    if (!projectPath || ctx.byProject.has(projectPath)) continue;
    ctx.byProject.set(projectPath, await reviewSignoff.loadSignoffs(projectPath));
  }
  return ctx;
}

// The C/L rating a maintainer left at the end of each review file.
//
// Deliberately NOT gated on the sign-off check, unlike the round trip. This
// team closes a review by *deleting* the file ("close out review for MR !227
// -- all items fixed"), so requiring it to still be on disk would throw away
// the rating for exactly the merge requests that were reviewed most
// thoroughly. The rating is read from history wherever the file last lived.
//
// The two rules pull in opposite directions on purpose: deleting the file
// removes the round-trip mark (it is a judgement someone withdrew), while the
// difficulty rating it recorded survives (it is a fact about the work).
//
// Read from the exact commit that added the file (readCommittedBody), never
// from whatever is on disk right now. coder-review rewrites review/MR-<iid>.md
// on its own subsequent runs — reading the working tree here would let the
// tool's next run silently overwrite the committed rating in the score, with
// nothing on screen showing it happened.
//
// Only MRs whose report was ever committed are read, so this costs a git call
// per reviewed MR, not per MR.
async function loadRatings(ctx, entries) {
  const ratings = new Map();
  await Promise.all(entries.map(async ({ mr, configuredId }) => {
    const projectPath = projects.getProjectPath(configuredId) || secret('PROJECT_PATH');
    const signoffs = ctx.byProject.get(projectPath);
    const added = signoffs && signoffs.get(Number(mr.iid));
    if (!added) return;
    // The verdict comes from who added the file; the rating comes from the
    // newest revision of it, because the review process rewrites the C/L line
    // each round. That newest revision must itself be maintainer-authored —
    // otherwise a developer could rate their own merge request by editing the
    // report after a maintainer created it.
    const newest = added.latest || added;
    const source = reviewSignoff.isMaintainerAuthor(newest, ctx.maintainers) ? newest : added;
    const body = await reviewSignoff.readCommittedBody(projectPath, source.sha, source.path);
    const rating = difficulty.parseRating(body);
    if (rating) ratings.set(mrKey(mr), rating);
  }));
  return ratings;
}

function signoffForMr(ctx, configuredProjectId, mrIid) {
  const projectPath = projects.getProjectPath(configuredProjectId) || secret('PROJECT_PATH');
  return reviewSignoff.signoffFor({
    iid: mrIid,
    exists: hasReviewReport(configuredProjectId, mrIid),
    signoffs: ctx.byProject.get(projectPath) || new Map(),
...
async function buildDeveloperAnalytics(authorUsername, { since, until } = {}) {
  ...
  const signoffCtx = await loadSignoffContext(projectIds);
  // Only MRs a maintainer actually signed off are worth spending a commits
  // lookup on: they are the only ones that can produce a round-trip verdict.
  const eligibleForCheck = entries.filter((e) => signoffForMr(signoffCtx, e.configuredId, e.mr.iid).counts);
  // Ratings span every MR whose report was ever committed — including the
  // ones whose file has since been deleted, which the sign-off gate excludes.
  const ratings = await loadRatings(signoffCtx, entries);
  const checked = eligibleForCheck.slice(0, MAX_MRS_FOR_BRANCH_CHECK);
  const commitInfoByMr = new Map();
  await Promise.all(checked.map(async ({ mr }) => {
    try {
      // One call, three answers: who committed (round-trip), when the branch
      // started (cycle time), and how many commits landed after the MR was
      // opened (rework). Fetching these separately would triple the cost of
      // the slowest part of this page.
      commitInfoByMr.set(mrKey(mr), await gitlab.getMergeRequestCommitStats(mr.project_id, mr.iid));
    } catch (e) {
      commitInfoByMr.set(mrKey(mr), null); // API error — unknown, not "no"
    }
  }));

  const records = entries.map(({ mr, configuredId }) => {
    const key = mrKey(mr);
    const signoff = signoffForMr(signoffCtx, configuredId, mr.iid);
    const reviewed = signoff.counts;
    const info = commitInfoByMr.has(key) ? commitInfoByMr.get(key) : undefined;
    const authors = info ? info.byAuthor : (info === null ? null : undefined);
    ...
    return {
      ...
      webUrl: mr.web_url,
      hasReport: reviewed,
      // Why this MR does or does not carry a round-trip verdict. Without it,
      // an MR that quietly stopped counting is indistinguishable from one
      // that was never looked at.
      signoff: { counts: signoff.counts, reason: signoff.reason, by: signoff.by || null, at: signoff.at || null },
      // The maintainer's C/L rating for this MR, when they left one.
      rating: ratings.get(key) || null,
      commitAuthorCount: known ? authors.length : null,
      otherAuthors: known ? others.map((a) => a.name) : null,
      roundTrip,
      // Delivery metrics (lib/deliveryMetrics.js). notesCount comes free on
      // the MR list; the commit-derived ones only exist for MRs whose
      // commits were fetched above, and are null rather than 0 otherwise —
      // "not measured" must not read as "nothing happened".
      notesCount: typeof mr.user_notes_count === 'number' ? mr.user_notes_count : null,
      firstCommitAt: info ? info.firstAt : null,
      lastCommitAt: info ? info.lastAt : null,
      commitCount: info ? info.total : null,
      commitsAfterOpen: info && mr.created_at
        ? info.times.filter((t) => t > Date.parse(mr.created_at)).length
        : null,
    };
  });
  ...
  return {
    ...
    // Straight from GitLab, so complete for every MR rather than only the
    // ones somebody remembered to fill a field on.
    delivery: deliveryMetrics.summarize(records),
    ratings: records.filter((r) => r.rating).map((r) => ({ iid: r.iid, ...r.rating })),
    months,
  };
}
