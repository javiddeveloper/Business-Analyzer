...
function buildMonthlyRows({ analytics = {}, jiraTasks = [], reviews = [], now = Date.now() } = {}) {
  ...
  // Reviews are grouped by the month they ran in, and each month's slice is
  // deduped to one record per merge request (devScore.dedupeReviews) before
  // it is counted or scored — re-running a review five times on one MR is a
  // click count, not five months' worth of evidence.
  ...
  return Array.from(months.values())
    ...
    .map((row) => {
      ...
      return {
        ...
        reviewCount: devScore.dedupeReviews(row.reviews).length,
        score: score.score,
...
const COLUMNS = [
  ...
  { key: 'reviewCount', label: 'تعداد ریویو', width: 13, format: 'int',
    hint: 'MRهایی که این ابزار در آن ماه برایشان ریویو ثبت کرده. اجرای چندباره‌ی ریویو روی یک MR یک بار شمرده می‌شود.' },
  ...
  { key: 'c_codeQuality', label: 'نمره: کیفیت کد', width: 18, format: 'int',
    hint: 'بر اساس آخرین ریویوی هر MR، با وزن High=۳، Medium=۱٫۵، Low=۰٫۵، تقسیم بر جذر تعداد فایل‌های تغییرکرده (MR بزرگ‌تر طبیعتاً یافته‌ی بیشتری دارد). نمره نصف می‌شود به‌ازای هر ۳ واحد چگالی یافته، پس هیچ‌وقت روی صفر گیر نمی‌کند.' },
  ...
  { key: 'c_reworkFree', label: 'نمره: برنچ تک‌نویسنده', width: 22, format: 'int',
    hint: 'چند درصد MRها فقط کامیت خودِ نویسنده را دارند. وزن این مؤلفه فقط ۵٪ است: در این تیم کامیت دیگران روی برنچ رایج است، پس بیشتر توصیف روش کار است تا نمره‌ی فرد.' },
...
// The manager's rating, as rows. Kept as its own function because it is the
// one block on this sheet that is a human's opinion rather than a
// measurement, and it should stay easy to see that in the code too.
function managerBlock(manager, loud) {
  if (!manager) {
    return [[loud('امتیاز مدیر'), loud(''),
      loud('مدیر هنوز برای این فرد امتیازی ثبت نکرده — این خانه خالی است، نه صفر.')]];
  }
  const rows = [[
    loud('امتیاز مدیر'),
    loud(manager.overall == null ? '' : manager.overall),
    loud('از ۱۰۰ — داوری مستقیم مدیر برای ' + faMonthLabel(manager.month)
      + '. مستقل از امتیاز خودکار است و در آن اثری ندارد.'),
  ]];
  for (const p of manager.params) {
    rows.push(['• ' + p.label, p.value == null ? '' : p.value, p.value == null ? 'امتیاز نداده' : 'از ۵']);
  }
  if (manager.note) rows.push(['• یادداشت مدیر', '', manager.note]);
  return rows;
}
...
function summaryRows({ author, analytics, manager = null, hoursLabel = (h) => (h == null ? '' : h) }) {
  const d = analytics.delivery || {};
  const sp = analytics.latestSprint;
  const score = analytics.autoScore || {};
  // Whoever opens this workbook is reading it to find the manager's verdict,
  // so it sits above the computed numbers and carries a style of its own. The
  // emphasis is attached to these cells rather than to a row index, so later
  // inserts here can't move it onto something else.
  const loud = (v) => ({ v, s: 'strong' });
  const rows = [
    ['مورد', 'مقدار', 'توضیح'],
    ['توسعه‌دهنده', author, ''],
    ...managerBlock(manager, loud),
    [],
    // Named "خودکار", not "کل": with a manager's score on the sheet right
    // above it, "امتیاز کل" would read as if it already included that.
    ['امتیاز خودکار (محاسبه‌شده)', score.score == null ? '' : score.score, score.reason || ''],
    [],
