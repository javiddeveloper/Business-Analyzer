...
// This tool's own output. review/MR-<iid>.md is written by reportFile.js and
// then, once committed, shows up in the very merge request it describes —
// so a re-review reads the previous round's report back as if it were part
// of the change under review.
//
// Three costs, all of which were visible on MR !191: tokens spent re-reading
// a report the tool wrote itself; the file count inflated, which feeds the
// "this MR is too big" check; and the model left free to raise findings
// about the wording of its own prior review. None of that is review of the
// author's code, which is the only thing this pipeline is for.
const SKIP_OWN_OUTPUT = /(^|\/)review\/MR-\d+\.md$/i;
...
function classify(change) {
  ...
  if (SKIP_OWN_OUTPUT.test(p)) return { skip: true, reason: 'گزارش ریویوی خودِ این ابزار' };
