...
// secrets.env (see lib/envFile.js), defaulting to Iran's official weekend
// (Thursday/Friday) when it isn't set. Exists so a "how late is this,
// really" question can be answered in working days instead of flat
// calendar days: a task due Wednesday and delivered the following Sunday is
// one working day late here, not four.
//
// Deliberately does nothing yet beyond the weekly pattern — no public-
// holiday list is wired in (confirmed with the user: for now the fixed
// Thu/Fri weekend is enough; a real holiday calendar is a later addition
// once there's a named source for one — Jira has no built-in company
// holiday calendar without a marketplace add-on). See workingDaysBetween's
// own comment.
...
// Unset means Iran's standard week (Sat-Wed working, Thu/Fri off) — asked
// for explicitly rather than left as "every day counts": nobody actually
// runs a 7-day work week, and defaulting to one would just mean the
// working-day math is silently wrong until someone finds and fills in a
// setting they didn't know existed. Still fully overridable from settings.
function workingDayIndices() {
  const raw = secret('WORKING_DAYS');
  if (!raw || !raw.trim()) return DEFAULT_WORKING_DAYS;
...
// of `to`) — the unit "how many working days late" needs.
//
// Holidays are NOT subtracted — only the weekly pattern is. A public holiday
// still turns up as a working day here, which understates how much slack a
// person actually had; add a real list once there's a named source for one.
