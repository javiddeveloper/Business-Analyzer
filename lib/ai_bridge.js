...
// projectId is optional and only matters for the handful of keys a project
// can override (GITLAB_TOKEN and friends — see lib/projects.js's
// OVERRIDE_PATH). Omitting it, or asking for a key no project can override,
// is exactly today's behaviour: secrets.env, then process.env. Required
// lazily, through the module object rather than a destructured binding, for
// the same reason budgetFor below is: lib/projects.js calls back into this
// module's own `secret` (for the synthesized .env project), so whichever of
// the two loads first must not need the other to already be finished.
function secret(key, projectId) {
  if (projectId) {
    try {
      const override = require('./projects').connectionOverride(projectId, key);
...
// `claude --effort <level>` accepts exactly these five. An unrecognised value
// makes the CLI exit non-zero, which would surface as an opaque review
// failure, so a typo is dropped here (and reported once) instead of being
// forwarded. Empty means "don't pass the flag at all" — the CLI's own default.
const CLAUDE_EFFORT_LEVELS = ['low', 'medium', 'high', 'xhigh', 'max'];
let warnedBadEffort = false;

function claudeEffort() {
  const raw = (secret('CLAUDE_EFFORT') || '').trim().toLowerCase();
  if (!raw) return '';
  if (CLAUDE_EFFORT_LEVELS.includes(raw)) return raw;
  if (!warnedBadEffort) {
    warnedBadEffort = true;
    console.warn(`CLAUDE_EFFORT="${raw}" شناخته نشد — نادیده گرفته شد. ` +
      `مقادیر مجاز: ${CLAUDE_EFFORT_LEVELS.join('، ')}`);
  }
  return '';
}

async function callClaudeCli({ system, user, maxTokens, signal, onDelta }) {
  const prompt = system ? `${system}\n\n---\n\n${user}` : user;
  const model = secret('CLAUDE_MODEL');
  const effort = claudeEffort();
  // stream-json only when somebody is listening. The two formats disagree
  // about stdout shape — one JSON document vs NDJSON — and asking for the
  // streaming one when nothing reads the deltas would mean parsing a harder
  // format for no benefit, on every code path including the engine test.
  ...
  return new Promise((resolve) => {
    ...
    // In streaming mode stdout is NDJSON and arrives in arbitrary chunks, so
    // lines are reassembled across reads and the tail is held back until a
    // newline proves it complete. `out` still collects everything, so the
    // non-streaming parse below is untouched.
    let pending = '';
    ...
    const handleLine = (line) => {
      ...
      if (!t) return;
      let ev;
      try { ev = JSON.parse(t); } catch (e) { return; } // a partial or non-JSON line is not fatal
      ...
      if (ev.type === 'stream_event' && ev.event && ev.event.type === 'content_block_delta') {
        ...
        if (d && typeof d.text === 'string' && d.text) {
          try { onDelta(d.text); } catch (e) { /* a broken listener must not kill the review */ }
        }
      }
    };
    ...
    child.on('close', () => {
      ...
      settled = true;
      try {
        if (streaming && pending.trim()) handleLine(pending); // last line, if it had no trailing newline
        // In streaming mode the whole of stdout is NDJSON and will not
        // JSON.parse; the `result` event carries the same fields the
        // single-document format used to.
        const r = streaming ? streamResult : JSON.parse(out);
        if (!r) throw new Error('no result event');
...
// Icons match the same choices the dashboard's mock fixture uses for these
// four engines (public/admin.html), so the demo and the real toolbar picker
// look like the same product rather than one of them still carrying the
// raw emoji this project otherwise replaced everywhere else.
const ENGINES = [
  { id: 'claude-cli', label: 'Claude', icon: '<i class="ico i-bot ico-violet"></i>', hint: 'اشتراک/لاگین claude CLI — بدون API key' },
  { id: 'openai-compatible', label: 'GapGPT', icon: '<i class="ico i-zap ico-amber"></i>', hint: 'یا هر اندپوینت دیگر سازگار با OpenAI' },
  { id: '9router', label: '9Router', icon: '<i class="ico i-git-branch ico-cyan"></i>', hint: 'روتر چندمدله (پیش‌فرض: http://localhost:20128/v1)' },
  { id: 'gemini', label: 'Gemini', icon: '<i class="ico i-sparkles ico-violet"></i>', hint: 'Google Generative Language API' },
];
