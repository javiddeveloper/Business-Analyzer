...
function envProject() {
  ...
  if (!id) return null;
  return { id: String(id), name: 'پیش‌فرض (.env)', path: secret('PROJECT_PATH') || '', fromEnv: true };
}
...
function upsertProject({ id, name, path: repoPath }) {
  if (!id) throw new Error('شناسه پروژه (GitLab project id) لازم است');
  const raw = readRaw();
  const list = Array.isArray(raw.list) ? raw.list : [];
  const idx = list.findIndex((p) => String(p.id) === String(id));
  const entry = { id: String(id), name: String(name || '').trim() || `پروژه ${id}`, path: String(repoPath || '').trim() };
...
module.exports = { listProjects, getProject, getProjectPath, getActiveProjectId, setActiveProjectId, upsertProject, removeProject };
