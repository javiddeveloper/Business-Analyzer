...
const SCHEMA = [
  ...
  // (lib/jira.js). JIRA_PROJECT_KEY isn't read anywhere yet — task keys are
  // matched by the full key extracted from the branch/title (task.js), not
  // by project prefix — kept here for the next Jira feature to use.
  { key: 'JIRA_BASE_URL', group: 'جیرا', secret: false, hint: 'مثلاً https://yourteam.atlassian.net' },
  { key: 'JIRA_EMAIL', group: 'جیرا', secret: false, hint: 'ایمیل حساب جیرا (برای Basic Auth با API token)' },
  { key: 'JIRA_API_TOKEN', group: 'جیرا', secret: true, hint: 'از id.atlassian.com/manage-profile/security/api-tokens' },
  { key: 'JIRA_PROJECT_KEY', group: 'جیرا', secret: false, hint: 'مثلاً EM — فعلاً استفاده نمی‌شود، برای قابلیت‌های بعدی' },
];
