...
// frozen top row, a native (live, not an image) column chart, and a way for a
// single cell to shout louder than its neighbours.
//
// Deliberately NOT implemented: shared strings (inline strings cost a few
// more bytes and remove a whole index to keep consistent), merged cells,
// formulas. Add them when something needs them.
...
// Column formats a caller can ask for by name. Kept small on purpose — the
// point is that a reader can tell 7 hours from 7.4 hours and that a count
// never shows up as "13.0", not that the workbook looks designed.
const FORMAT_STYLE = { text: 0, int: 2, decimal: 3, wrap: 4 };
// Styles a cell can ask for by name, independent of its column's format.
const NAMED_STYLE = { strong: 5 };

function sheetXml(rows, { freezeHeader = true, columns = [], autoFilter = false, withDrawing = false } = {}) {
  const width = Math.max(0, ...rows.map((r) => r.length));
  const styleFor = (c) => FORMAT_STYLE[(columns[c] && columns[c].format) || 'text'] || 0;

  const body = rows.map((row, r) => {
    const cells = row.map((cell, c) => {
      const ref = `${colName(c)}${r + 1}`;
      // A cell may be given as { v, s } to ask for a style of its own. That is
      // addressed to the cell rather than to a row index, so inserting a row
      // above it cannot move the emphasis onto something else.
      const named = cell && typeof cell === 'object' && !Array.isArray(cell) && 'v' in cell;
      const value = named ? cell.v : cell;
      const style = r === 0 ? 1 : (named && NAMED_STYLE[cell.s] != null ? NAMED_STYLE[cell.s] : styleFor(c));
      const styleAttr = style ? ` s="${style}"` : '';
      // Written as an empty cell, never as 0 or "—": a blank reads as "not
      // measured", and a zero would be a claim.
    ...
    // A row carrying an emphasised cell gets the height that cell's larger
    // font needs. Derived from the row's own cells, so it follows the emphasis
    // around instead of being pinned to a row number.
    const tall = row.some((cell) => cell && typeof cell === 'object' && NAMED_STYLE[cell.s] != null);
    return `<row r="${r + 1}"${tall ? ' ht="24" customHeight="1"' : ''}>${cells}</row>`;
  }).join('');

  const widths = Array.from({ length: width }, (_, c) => {
    const w = (columns[c] && columns[c].width) || 18;
  ...
  // Header dropdowns, so a sheet with a year of months can be filtered
  // without anyone reaching for the ribbon.
  const filter = autoFilter && rows.length > 1 && width
    ? `<autoFilter ref="A1:${colName(width - 1)}${rows.length}"/>`
    : '';

  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">${pane}<cols>${widths}</cols><sheetData>${body}</sheetData>${filter}${withDrawing ? '<drawing r:id="rId1"/>' : ''}</worksheet>`;
}

const STYLES_XML = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><numFmts count="2"><numFmt numFmtId="164" formatCode="#,##0"/><numFmt numFmtId="165" formatCode="0.0"/></numFmts><fonts count="3"><font><sz val="11"/><name val="Calibri"/></font><font><b/><sz val="11"/><name val="Calibri"/></font><font><b/><sz val="14"/><color rgb="FF7A4F01"/><name val="Calibri"/></font></fonts><fills count="4"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FFE8EEF7"/><bgColor indexed="64"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFFFF3CD"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="1"><border><left/><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="6"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment vertical="center" wrapText="1"/></xf><xf numFmtId="164" fontId="0" fillId="0" borderId="0" xfId="0" applyNumberFormat="1"/><xf numFmtId="165" fontId="0" fillId="0" borderId="0" xfId="0" applyNumberFormat="1"/><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0" applyAlignment="1"><alignment vertical="top" wrapText="1"/></xf><xf numFmtId="0" fontId="2" fillId="3" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment vertical="center" wrapText="1"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles><dxfs count="0"/></styleSheet>`;
