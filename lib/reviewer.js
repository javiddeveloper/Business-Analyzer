...
// run), because the reader is the same in both: the MR author, and the
// Persian report file.
//
// The prompts are written in Persian, which is not the same as asking for a
// Persian answer — over an English/Kotlin codebase the model follows the
// code and answers in English, which is exactly what MR !191 produced. The
// carve-out matters as much as the rule: translating an identifier or a path
// makes a finding impossible to act on, since the thing it names no longer
// matches anything in the repository.
const PERSIAN_OUTPUT_RULE =
  'همه‌ی متن‌های خروجی (summary، title، note، suggestion، positives) را فارسی بنویس، حتی وقتی کد و نام فایل‌ها انگلیسی‌اند. ' +
  'استثنا: نام فایل، مسیر، نام تابع/کلاس/متغیر، پیام خطا و تکه‌کد را دقیقاً به همان شکل اصلی و انگلیسی بگذار — ترجمه‌شان یافته را غیرقابل‌پیگیری می‌کند.';
...
module.exports = { review, prepareFiles, buildBatches, buildUserPrompt, normalizeFindings, normalizeRating, decide, parseJson, sortFindings, dedupeFindings, coverageStats, mapWithConcurrency, MAX_BATCHES, BATCH_CONCURRENCY, PERSIAN_OUTPUT_RULE };
