...
function buildBatches(files) {
  ...
  for (const file of files) {
    const chunkSize = file.annotated.length;
...
function buildUserPrompt({ mr, batch, batchIndex, batchCount }) {
  const body = batch
    .map((f) => `### ${f.path}${f.isNew ? ' (فایل جدید)' : ''}${f.truncated ? ' (بریده‌شده)' : ''}\n${f.annotated}`)
    .join('\n\n');
...
async function callForJson({ system, user }) {
  const first = await callModel({ system, user, maxTokens: 3000 });
...
async function review({ mr, changes }) {
  const { files, skipped } = prepareFiles(changes || []);
  ...
  const results = await Promise.all(
    batches.map((batch, i) =>
      callForJson({ system, user: buildUserPrompt({ mr, batch, batchIndex: i, batchCount: batches.length }) })
        .then((r) => ({ ...r, batch }))
        .catch((e) => ({ error: e.message, batch }))
    )
  );
