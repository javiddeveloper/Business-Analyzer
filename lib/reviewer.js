...
async function callForJson({ system, user, signal, provider, onDelta }) {
  // Only the first attempt streams. The repair call re-emits the same content
  // as valid JSON, so streaming it would replay the whole answer a second
  // time into a panel that already showed it.
  const first = await callModel({ system, user, maxTokens: outputBudget(provider), signal, provider, onDelta });
...
async function review({ mr, changes, fileContents, signal, provider, jiraIssue, onDelta }) {
  ...
  const results = await mapWithConcurrency(batches, BATCH_CONCURRENCY, (batch, i) =>
    callForJson({
      system,
      user: buildUserPrompt({ mr, batch, batchIndex: i, batchCount: batches.length, jiraIssue }),
      signal,
      provider,
      // Tagged with the batch it came from. Batches run BATCH_CONCURRENCY at a
      // time, so without the index every stream would land in one buffer and
      // six models would write over each other.
      onDelta: onDelta ? (text) => onDelta(text, i, batches.length) : undefined,
    })
      .then((r) => ({ ...r, batch }))
      .catch((e) => {
