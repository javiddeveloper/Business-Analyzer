...
async function callForJson({ system, user, signal, provider }) {
  const first = await callModel({ system, user, maxTokens: 3000, signal, provider });
...
async function review({ mr, changes, fileContents, signal, provider }) {
  ...
  const results = await mapWithConcurrency(batches, BATCH_CONCURRENCY, (batch, i) =>
    callForJson({ system, user: buildUserPrompt({ mr, batch, batchIndex: i, batchCount: batches.length }), signal, provider })
      .then((r) => ({ ...r, batch }))
      .catch((e) => {
...
module.exports = { review, prepareFiles, buildBatches, normalizeFindings, decide, parseJson, sortFindings, mapWithConcurrency, MAX_BATCHES, BATCH_CONCURRENCY };
