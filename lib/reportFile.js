...
// The same location as a repository-relative path, for the GitLab commit API
// (which wants `review/MR-12.md`, never a Windows absolute path). Forward
// slashes always: this is a git path, not a filesystem one.
function reportRelPath(mrIid) {
  return `review/MR-${mrIid}.md`;
}
...
function buildMarkdown({ mrIid, mr, result, jiraIssue, commitStats }) {
  ...
  // Bare, and last. The team's own process file (review/README.md) is
  // explicit that this line must be "a bare C<stars> L<stars> token, nothing
  // else… it must not be explained, titled, expanded, or called out anywhere
  // in the report itself". This used to print a legend above it
  // ("C = پیچیدگی · L = حجم کدی که باید خوانده شود"), which broke that rule in
  // the one file that travels to the branch — spelling out an internal
  // estimate for anyone who opens the merge request. The meaning belongs in
  // lib/difficulty.js and on the dashboard, not in the pushed artifact.
...
// Returns { path, content } on success, or { path: null, error } if the write
// failed (e.g. no permission, disk full) — a report-writing failure must not
// fail the whole review, since the GitLab comment/dashboard result is still
// valid. `content` comes back so a caller that also pushes the report to the
// branch commits the exact bytes that were written here, rather than
// rebuilding the markdown and risking the two drifting (the footer alone
// carries a timestamp, so a rebuild is never byte-identical).
function writeReport({ projectPath, mrIid, mr, result, jiraIssue, commitStats }) {
  try {
    ...
    const content = buildMarkdown({ mrIid, mr, result, jiraIssue, commitStats });
...
module.exports = { writeReport, buildMarkdown, reportPath, reportRelPath, reportDir };
