...
// own Sentry (lib/projects.js connections.sentry) — a different install
// entirely, not just a different project slug on the same one. Omitted,
// every function here is exactly the single-Sentry behaviour it always was.
async function apiFetch(pathname, { timeoutMs = DEFAULT_TIMEOUT_MS, projectId } = {}) {
  const base = sentryBase(projectId);
  ...
  try {
    res = await fetch(`${base}/api/0${pathname}`, {
      headers: { Authorization: `Bearer ${secret('SENTRY_AUTH_TOKEN', projectId)}`, Accept: 'application/json' },
...
function sentryBase(projectId) {
  return (secret('SENTRY_URL', projectId) || '').replace(/\/+$/, '');
}

function isConfigured(projectId) {
  return !!(sentryBase(projectId) && secret('SENTRY_AUTH_TOKEN', projectId) && secret('SENTRY_ORG', projectId) && projectSlugs(projectId).length);
}
...
function projectSlugs(projectId) {
  return (secret('SENTRY_PROJECT', projectId) || '')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
}
...
// the browser polls.
function normalizeIssue(raw, projectSlug, projectId) {
  ...
  return {
    ...
    permalink: raw.permalink || `${sentryBase(projectId)}/organizations/${secret('SENTRY_ORG', projectId)}/issues/${raw.id}/`,
  };
}
...
async function listIssues({ query = 'is:unresolved', statsPeriod = '14d', limit = 50, projects, projectId } = {}) {
  const slugs = projects && projects.length ? projects : projectSlugs(projectId);
  const org = secret('SENTRY_ORG', projectId);
  const issues = [];
  const errors = [];
  ...
  await Promise.all(slugs.map(async (slug) => {
    ...
    try {
      const raw = await apiFetch(`/projects/${encodeURIComponent(org)}/${encodeURIComponent(slug)}/issues/?${qs}`, { projectId });
      for (const item of Array.isArray(raw) ? raw : []) issues.push(normalizeIssue(item, slug, projectId));
    } catch (e) {
...
async function issueDetail(issueId, projectId) {
  const [issue, event] = await Promise.all([
    apiFetch(`/issues/${encodeURIComponent(issueId)}/`, { projectId }),
    // A brand-new issue can briefly have no stored event; that is not fatal,
    // the list data alone is still worth showing.
    apiFetch(`/issues/${encodeURIComponent(issueId)}/events/latest/`, { projectId }).catch(() => null),
  ]);
  ...
  return {
    ...normalizeIssue(issue, (issue.project && issue.project.slug) || null, projectId),
...
async function setIssueStatus(issueId, status, projectId) {
  ...
  try {
    res = await fetch(`${base}/api/0/issues/${encodeURIComponent(issueId)}/`, {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${secret('SENTRY_AUTH_TOKEN', projectId)}`,
...
async function checkConnection(projectId) {
  const org = secret('SENTRY_ORG', projectId);
  const info = await apiFetch(`/organizations/${encodeURIComponent(org)}/`, { timeoutMs: 8000, projectId });
