...
// The task body. Written so somebody who never opens Sentry still knows what
// they are fixing, with the permalink for when they do.
function buildDescription(issue) {
  ...
  lines.push(
    ...
    `*پروژه:* ${issue.project || '—'}`,
    '',
    `لینک در Sentry: ${issue.permalink}`,
  );
  return lines.join('\n');
}
