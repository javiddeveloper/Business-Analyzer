...
async function fetchIssueSafe(issueKey, { force = false } = {}) {
  if (!isConfigured() || !issueKey) return null;
  try {
    const { value } = await cache.cached('jira-issue', issueKey, ISSUE_CACHE_TTL_MS, () => fetchIssue(issueKey), { force });
    return value;
  } catch (e) {
    return null;
  }
...
const SEARCH_FIELDS = [
  'summary', 'status', 'assignee', 'updated', 'created', 'issuetype', 'priority',
...
async function searchIssuesByAssignee(username, { since, until, force = false } = {}) {
  ...
  try {
    // Cache name carries a version, bumped whenever the cached shape changes
    // — v1 held a bare array (reading it back after the upgrade to
    // { issues, total } crashed the analytics page on `.issues.map`), v3 adds
    // the estimate/due-date/sprint fields the score is built on. Without the
    // bump, an upgrade serves entries that are silently missing the new
    // fields, which reads as "this developer logs no estimates" rather than
    // as a stale cache. The normalize step below is the belt to that
    // suspenders: whatever comes out, the caller gets { issues, total }.
    const { value } = await cache.cached('jira-assignee-v3', cacheKey, SEARCH_CACHE_TTL_MS, async () => {
