...
async function createIssue({ summary, description, issueType = 'Bug', dueDate, estimateHours, labels }) {
  if (!canCreate()) throw new Error('جیرا برای ساخت تسک تنظیم نشده — JIRA_BASE_URL و JIRA_API_TOKEN و JIRA_PROJECT_KEY لازم‌اند.');
  ...
  const fields = {
    project: { key: secret('JIRA_PROJECT_KEY') },
...
module.exports = { ISSUE_CACHE, SEARCH_CACHE, isConfigured, canCreate, fetchIssue, fetchIssueSafe, fetchIssuesByKeys, searchIssuesByAssignee, createIssue, toEstimateString };
